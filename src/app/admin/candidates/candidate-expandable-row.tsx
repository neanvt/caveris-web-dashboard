...
export function MatchScore({ pct, result, isStatusOnly = false, captured = false }: { pct?: number | null; result: string; isStatusOnly?: boolean; captured?: boolean }) {
  ...
  const colorClass = isOk ? "from-green-400 to-emerald-500 text-white" : score != null && score >= 60 ? "from-yellow-400 to-orange-400 text-white" : "from-red-400 to-rose-500 text-white";

  return (
    <div className="flex flex-col items-center gap-1">
      <div className={`flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br ${colorClass} shadow-md`}>
        <span className="text-sm font-bold">{score != null ? `${score}%` : isOk ? "✓" : "✗"}</span>
      </div>
      <span className={`text-[10px] font-semibold uppercase ${isOk ? "text-green-600" : "text-red-500"}`}>{isOk ? "Match" : "No Match"}</span>
    </div>
  );
}
...
export function StoredThumbMini({ src, icon, color, title }: any) {
  ...
  const [errored, setErrored] = useState(false);
  const hasData = !!src;
  const colorMap: any = { blue: "ring-blue-300 bg-blue-50", indigo: "ring-indigo-300 bg-indigo-50", purple: "ring-purple-300 bg-purple-50" };
  const iconColorMap: any = { blue: "text-blue-400", indigo: "text-indigo-400", purple: "text-purple-400" };
  ...
  return (
    <div title={title} className={`relative h-10 w-10 overflow-hidden rounded-lg ring-1 ${hasData ? colorMap[color] : "ring-gray-200 bg-gray-50"}`}>
      {resolved && !errored && hasData ? (
        <img src={resolved} alt={title} className="h-full w-full object-cover" onError={() => setErrored(true)} />
      ) : (
        <div className={`flex h-full w-full items-center justify-center ${hasData ? iconColorMap[color] : "text-gray-200"}`}>{icon}</div>
      )}
...
export function CandidateExpandableRow({ candidate, verifications, onView, onEdit, isTestingExam }: {
  candidate: any;
  verifications: any[];
  onView: () => void;
  onEdit: () => void;
      ...
      <tr className={`border-b transition-colors hover:bg-gray-50 ${expanded ? "bg-indigo-50/40" : ""}`}>
        <td className="px-4 py-3">
          ...
          >
            ...
            <span className="ml-1 rounded-full bg-gray-100 px-1.5 py-0.5 text-[11px]">{cvs.length}</span>
          </button>
        </td>
        ...
        <td className="px-4 py-3">
          ...
          {candidate.father_name && <p className="text-xs text-gray-400">S/o {candidate.father_name}</p>}
        </td>
