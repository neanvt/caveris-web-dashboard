...
type EditVerifierFormValues = z.infer<typeof editVerifierSchema>;
...
interface EditVerifierModalProps {
  open: boolean;
  onClose: () => void;
...
export function EditVerifierModal({
  open,
  onClose,
  verifier,
  onSuccess,
  ...
  useEffect(() => {
    if (verifier) {
      form.reset({
        ...
        father_name: verifier.father_name,
        phone: verifier.phone,
        date_of_birth: verifier.date_of_birth || "",
        address: verifier.address || "",
        city: verifier.city || "",
        is_active: verifier.is_active ? "true" : "false",
      });
    }
  }, [verifier, form]);

  const onSubmit = async (values: EditVerifierFormValues) => {
    if (!verifier) return;
    ...
    try {
      ...
      toast({
        title: "Verifier Updated Successfully",
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      ...
      >
        <DialogHeader>
          <DialogTitle>Edit Verifier</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormItem>
              ...
              <Input value={verifier.email} disabled className="bg-gray-50" />
